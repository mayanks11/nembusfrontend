...
export class Minimap extends React.Component {
  ...
  handlerMouseScroll(event) {
    console.log(event,"handlerMouseScroll")
  }



  componentDidMount() {
    const { onMountCenterOnX, onMountCenterOnY ,canvas} = this.props
    setTimeout(() =>
      this.synchronize({
    ...
    // canvas.addEventListener("zoomUpdated", this.handlerMouseScroll, false);
